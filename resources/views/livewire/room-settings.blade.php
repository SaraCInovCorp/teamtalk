    ...
    <div class="p-8 max-w-xl">
        ...
        <form wire:submit.prevent="save" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="mb-4">
                ...
                <textarea wire:model.defer="description" id="description" rows="4" class="textarea textarea-bordered w-full"></textarea>
            ...
            <div class="mb-4 max-w-xs">
                ...
                <x-input type="number" wire:model.defer="message_delete_days" id="message_delete_days" min="0" class="input input-bordered w-full" />
                    ...
                    @endphp
                        ...
                        <li class="flex justify-between items-center py-1">
                            <span class="{{ $isBlocked ? 'text-red-600' : '' }}">
                                {{ $member->name }}
                                @if ($isAdmin)
                                    <span class="ml-2 text-green-600 font-semibold text-xs">
                                        (Admin da Sala)</span>
                                @endif
                            </span>
                            ...
                            <div class="flex space-x-2">
                                ...
                                <a 
                                    href="#" 
                                    wire:click.prevent="blockMember({{ $member->id }})" 
                                    title="Bloquear Membro" 
                                    aria-label="Bloquear membro" 
                                    class="inline-flex"
                                >
                                ...
                                @if ($member->pivot->role_in_room === 'admin')
                                    <a 
                                        href="#" 
                                        wire:click.prevent="toggleAdmin({{ $member->id }})" 
                                        title="Remover Admin" 
                                        aria-label="Remover administração do membro" 
                                        class="inline-flex"
                                    >
                                ...
                                @else
                                    <a 
                                        ...
                                        wire:click.prevent="toggleAdmin({{ $member->id }})" 
                                        title="Tornar Admin" 
                                        aria-label="Conceder administração ao membro" 
                                        class="inline-flex"
                                    >
                                        <x-icon size="w-6 h-6" color="#27ae60" class="cursor-pointer" >
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
